...
def audit_page(
    ...
    *,
    ...
    `rendered` — страницу открывали браузером, потому что сервер отдал пустой
    каркас. Это меняет смысл находки о сборке скриптами: проверка полная, и
    предупреждать о её неполноте больше не о чем.
    ...
    if rendered:
        # Страницу открыли браузером, и разбор полный. Сам факт сборки скриптами
        # при этом остаётся: посетитель на медленном телефоне ждёт дольше, а
        # счётчик и цели надо проверять отдельно. Это замечание к сайту, а не к
        # проверке, поэтому и уровень другой.
        issues.append(
            Issue(
                ...
                "Сервер отдаёт пустой каркас, содержимое дорисовывает браузер. Для "
                "проверки это не помеха — мы открыли страницу браузером, — но "
                "посетитель на медленном телефоне ждёт дольше, а срабатывание "
                "счётчика и целей на таких страницах стоит проверить отдельно.",
            )
        )
    elif looks_js_rendered(signals):
