...
class SuggestedGroupRead(BaseModel):
    """Группа, предложенная моделью.
    ...
    name: str
    #: Что общего у этих фраз — словами модели. Без объяснения предложение
    #: невозможно проверить, а непроверяемое предложение не принимают.
    reason: str = ""
    phrases: list[GroupPhraseRead] = Field(default_factory=list)
    total_frequency: int = 0
    #: Из каких групп фразы придут, если предложение принять. Пусто означает
    #: «сейчас они вне групп».
    moved_from: list[str] = Field(default_factory=list)


class GroupSuggestionRead(BaseModel):
    """Предложение модели по раскладке фраз.
    ...
    ok: bool = False
    #: Почему предложения нет. Пустое место без объяснения читается как поломка.
    reason: str = ""
    groups: list[SuggestedGroupRead] = Field(default_factory=list)
    #: Сколько групп сейчас — чтобы было с чем сравнивать.
    groups_now: int = 0
    #: Сколько фраз закреплено человеком: их модели не показывали.
    skipped_manual: int = 0
    #: Сколько фраз отдали модели. Список длиннее предела режется.
    considered: int = 0
    confidence: float = 0.0
    model: str = ""
