...
class ProgressInput:
    ...
    #: Задана ли ниша. Она не бюрократия: даёт стартовые минус-слова и включает
    #: проверку требований площадки — без лицензии клиника не пройдёт модерацию.
    has_niche: bool = False
...
def _onboarding(data: ProgressInput) -> Step:
    """Заведён ли проект по-настоящему.

    Раньше шаг закрывался фактом создания карточки. Это давало ощущение
    прогресса, которого нет: человек ввёл название и уже «один шаг из десяти».
    Формальность, оформленная как достижение, — та же выдуманная цифра, только
    в виде галочки.

    Теперь шаг закрывают два поля, каждое из которых что-то включает. Адрес
    сайта — единственное, из чего берётся исследование. Ниша — стартовые
    минус-слова и требования площадки, без которых клиника или продавец БАДов
    не пройдут модерацию.

    Шаг остаётся в работе, а не блокируется: реклама без сайта бывает — на
    квизы, на карточки в справочниках. Запретить такой проект нельзя, а не
    сказать вслух, что исследовать нечего, — можно ввести в заблуждение.
    """
    # Подсказки написаны по отдельности, а не собраны из кусков: склейка
    # «Укажите нишу — от них зависят…» читается как машинный перевод, а
    # подсказка, которую неприятно читать, перестаёт работать.
    if not data.has_website and not data.has_niche:
        return _step(
            StepKey.ONBOARDING,
            StepState.ACTIVE,
            "Укажите адрес сайта и нишу: без сайта нечего проверять, "
            "без ниши не с чего начинать минус-слова",
        )
    ...
    if not data.has_website:
        return _step(
            StepKey.ONBOARDING,
            StepState.ACTIVE,
            "Укажите адрес сайта — без него нечего проверять",
        )

    if not data.has_niche:
        return _step(
            StepKey.ONBOARDING,
            StepState.ACTIVE,
            "Выберите нишу — она даёт стартовые минус-слова и включает "
            "проверку требований Директа к этой отрасли",
        )

    return _step(StepKey.ONBOARDING, StepState.COMPLETED)
